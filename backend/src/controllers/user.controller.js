...
export default class UserController {
    ...
    getUserRegisterByValidation = async (req, res) => {
        try {
            ...
            if (!field || field === undefined || field === "") throw new Error("Campo vacío"); //Lanzamos un error si los campos no existen o están vacios
            ...
            if (!RESPONSE) return res.status(400).send(helper.generateRespond(this.#response, this.#valuesError)); 
            //Si existe pero no hay nada respondemos la petición
            if (RESPONSE.length !== 0 ) return res.status(200).send(helper.generateRespond(this.#response,[true, {exists: true}, ""] ));
            else return res.status(200).send(helper.generateRespond(this.#response,[true, {exists: false}, ""]));

        } catch (error) {
            //Modificamos el mensaje de error. 
            this.#valuesError[2] = error.message; 
            return res.status(400).send(helper.generateRespond(this.#response, this.#valuesError)); 
        }
    ...
    userLoginValidate = async (req, res)=>{
        try {
            ...
            const {user_id,username, password, email, profile_img}  = USERNAMEEXIST.length > 0 ? USERNAMEEXIST[0]:EMAILEXIST[0];
           //Teniendo el response haremos primero una comprobación con el password: 
            ...
            {   
                ...
                return res.status(200).send(helper.generateRespond(this.#response, [true,dataResponds,"" ]))
            }else
                return res.status(200).send(helper.generateRespond(this.#response, [true, dataResponds, ""]))
